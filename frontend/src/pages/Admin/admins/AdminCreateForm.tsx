...
import { Loader, Mail, User as UserIcon, Phone, ShieldCheck, X, Info } from "lucide-react";
import { motion } from "framer-motion";
import { addAdmin } from "../../../api/admin";

/**
 * AdminCreateForm — Home UI polished (dark / glass / neon)
 * - Mirrors the Home page tokens and backdrop feel
 * - Field-level validation + form error banner
 * - Subtle motion, strong focus rings, and accessible labels
 */
...
// ---- Theme tokens (kept in-file; consider moving to ui/theme.ts) ----
...
const AdminCreateForm: React.FC<Props> = ({ onSuccess, onCancel }) => {
  ...
  return (
    <div className="relative">
      {/* optional subtle grid/backlight like Home */}
      <div className="pointer-events-none absolute -z-10 -top-24 left-1/2 h-[36rem] w-[36rem] -translate-x-1/2 rounded-full opacity-20 blur-3xl" style={{ background: "radial-gradient(closest-side, rgba(34,211,238,0.35), transparent 70%)" }} />
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-bold tracking-tight">Create Admin</h2>
            <p className="text-sm text-slate-300/90">Invite a new administrator to your workspace.</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-start gap-2 text-slate-300/80 text-xs">
          ...
          <p>Admins can manage users, appointments, and billing. They’ll receive a verification email to activate access.</p>
        </div>
